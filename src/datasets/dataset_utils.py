...
def load_dataset_from_from_folder(in_path, cache_path, use_cache=True, image_size=None, use_cache_if_exists=False):
    """
...
def scan_folder_structure(directory):
    ...
    for root, dirs, files in os.walk(directory):
        for filename in files:
            if filename.lower().endswith(('.jpg', '.jpeg', '.gif', '.png', '.JPEG', ',tif')):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
        with tqdm.tqdm(total=len(image_paths)) as pbar_memory_load:
            for i, (image, image_path) in zip(range(len(image_paths)), 
                ...
                print(image_size, final_image_size)
...
def load_image(image_path, image_size=None):
    ...
    if image_size is not None:
        im = im.resize((h,w), Image.Resampling.LANCZOS)
    if c == 3 and im.mode == 'P':
        print("HERE: ", c)
        im = im.convert('RGB')

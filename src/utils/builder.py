...
class ExperimentBuilder():
    ...
    def load_from_checkpoint(self, checkpoint_name_or_path, load_model_only=False, load_backbone_only=False):
        ...
        if torch.cuda.is_available():
            ...
            print(best_gpu)
            map_location= lambda storage, location: storage.cuda(best_gpu.id)
        else:
            map_location='cpu'
    ...
    def save_checkpoint(self, checkpoint_name=None):
        ...
        if (self.args.model in ['matchingnet']) or ('ResNet' in self.args.backbone) or self.args.storage_friendly:
